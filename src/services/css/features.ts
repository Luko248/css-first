...
export const CSS_FEATURES: Record<string, CSSFeature> = {
  ...
  flexbox: {
    ...
    properties: ['display', 'flex-direction', 'justify-content', 'align-items', 'flex-wrap'],
  ...
  grid: {
    name: 'CSS Grid',
    category: CSSFeatureCategory.LAYOUT,
    properties: ['display', 'grid-template-columns', 'grid-template-rows', 'gap', 'grid-area'],
  ...
  'css-carousel': {
    name: 'CSS Carousel',
    category: CSSFeatureCategory.INTERACTION,
    properties: ['overflow-x', 'scroll-snap-type', 'scroll-snap-align', 'display'],
    description: 'Pure CSS carousel implementation using scroll snap and overflow',
    support_level: 'good',
  ...
  animations: {
    name: 'CSS Animations',
    category: CSSFeatureCategory.ANIMATION,
    properties: ['animation', 'keyframes', 'animation-duration', 'animation-timing-function'],
  ...
  transforms: {
    name: 'CSS Transforms',
    category: CSSFeatureCategory.VISUAL,
    properties: ['transform', 'transform-origin', 'scale', 'rotate', 'translate'],
...
export function searchFeatures(keywords: string[]): CSSFeature[] {
  ...
  for (const feature of Object.values(CSS_FEATURES)) {
    ...
    if (keywords.some(keyword => featureText.includes(keyword.toLowerCase()))) {
